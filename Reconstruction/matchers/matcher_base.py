...
from Reconstruction.extractors.extractor_base import ExtractorBase
...
class MatcherBase(metaclass=ABCMeta):
    ...
    def match(
        ...
        fallback_flag = False
        print(f"-------{self._tiling == TileSelection.NONE}----------")
        ...
        print(f"\n Image pair {img0.name}-{img1.name} having {num_inliers} matching")
...
class DetectorFreeMatcherBase(metaclass=ABCMeta):
    ...
    def __init__(self, custom_config) -> None:
        ...
        if self._config["general"]["tile_selection"] == TileSelection.PRESELECTION:
            self._preselction_extractor = (
                SuperPoint({"max_keypoints": 4048}).eval().to(self._device)
            )
            self._preselction_matcher = (
                LightGlue(
                    features="superpoint",
